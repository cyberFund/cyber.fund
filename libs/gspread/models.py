...
class Worksheet(object):
    ...
    def _fetch_cells(self):
        feed = self.client.get_cells_feed(self, params={'return-empty': 'true'})
    ...
    def _list_values_multi(self, index, cell_tuple, position):
        cells_list = self._fetch_cells()
        cells = dict(map(cell_tuple, filter(position, cells_list)))
    ...
    def cols(self, cols):
        """Returns a list of all cells in columns `cols`.
        ...
        cells_list = self._fetch_cells()
        cells = dict(map(
            lambda cell: ('R%sC%s' % (cell.row, cell.col), cell),
            filter(lambda cell: cell.col in cols, cells_list)))
        return cells.values()
